...
export default {
  ...
  shareDesign() {
    ...
    for (var i = 0; i < en0.length; i++) {
      if(en0[i]["family"] == "IPv4" && !en0[i]["internal"]) {
        networks.push(en0[i]);
      }
    }
    let wlan0 = ifaces["wlan0"] || [];
    for (var i = 0; i < wlan0.length; i++) {
      if(wlan0[i]["family"] == "IPv4" && !wlan0[i]["internal"]) {
        networks.push(wlan0[i]);
      }
    }
    let con = networks[0];
    if(this.port && this.majiView) {
    let url = "http://"+con["address"]+":"+this.port
    let _thisy = this;
    qrcode.toDataURL(url, function (err, data) {
        _thisy.majiView.showQRcode(data);
      })
    }
  },
  importDesign() {
