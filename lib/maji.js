...
export default {
  ...
  makeCoffeeScriptLink(name) {
    return app.replace("app","views/"+name);
  },
  ...
  importDesign() {
    ...
    if(files && files.length) {
       ...
       pagename = koth.sanitizeName(pagename);
       koth.createFramerViewfile(src,atompaths+"/views/"+pagename+".coffee");
       var cleanoptions = {
        files: atompaths+'/index.html',
        from: this.makeCoffeeScriptLink(pagename)+"\n\t",
        to: "",
       };
       var addoptions = {
        files: atompaths+'/index.html',
        from: app,
        to: this.makeCoffeeScriptLink(pagename)+"\n\t"+app,
      };
      replace.sync(cleanoptions);
      replace.sync(addoptions);
    }
    console.log(src);
  },
